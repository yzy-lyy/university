	...
	public ActionForward perform(ActionMapping mapping, ActionForm form,
		...
		if ("update".equals(actionFlag)) {
			...
			if (studentForm != null) {// 数据准备
				...
				studentvo.setUsername(CommonUtil.getLocalString(studentForm
						.getUsername()));
				studentvo.setGender(CommonUtil.getLocalString(studentForm
						.getGender()));
				studentvo.setCity(CommonUtil.getLocalString(studentForm
						.getCity()));
				studentvo.setIdentityID(CommonUtil.getLocalString(studentForm
						.getIdentityID()));
				studentvo.setPhone(CommonUtil.getLocalString(studentForm
						.getPhone()));
				studentvo.setEmail(CommonUtil.getLocalString(studentForm
						.getEmail()));
				studentvo.setPostID(CommonUtil.getLocalString(studentForm
						.getPostID()));
				studentvo.setAddress(CommonUtil.getLocalString(studentForm
						.getAddress()));
				studentvo.setStuID(CommonUtil.getLocalString(studentForm
						.getStuID()));
				try {
					LogUtil.logDebug("success1");
					StudentMgr stumgr = new StudentMgr();
					stumgr.updateStudent(studentvo);// 更新DB
				} catch (Exception e) {
			...
			try {
				StudentMgr stumgr = new StudentMgr();
				studentvo = stumgr.getStudentByID(stuid);
			} catch (Exception e) {
				System.out.println("error");
				// TODO Auto-generated catch block
				e.printStackTrace();
